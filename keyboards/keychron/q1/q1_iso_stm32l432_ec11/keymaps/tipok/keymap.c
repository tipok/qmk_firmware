...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_iso_83(
         ...
         KC_LCTL,  KC_LOPTN, KC_LCMMD,                               KC_SPC,                                 KC_RCMMD, KC_RALT, MO(MAC_FN),  KC_LEFT,  KC_DOWN,  KC_RGHT),

    [MAC_FN] = LAYOUT_iso_83(
         _______,  KC_BRMD,  KC_BRMU,  _______,  _______,  RGB_VAD,  RGB_VAI,  RGB_TOG,  RGB_MOD, RGB_RMOD,  RGB_M_P,  _______,    _______,  _______,            RESET,
         _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,    _______,  KC_DEL,             KC_LCKS,
...
layer_state_t layer_state_set_user(layer_state_t state) {
    if (layer_state_cmp(state, MAC_BASE)) {
        current_layer = MAC_BASE;
    }
    if (layer_state_cmp(state, MAC_FN)) {
        current_layer = MAC_FN;
    }
    if (layer_state_cmp(state, WIN_BASE)) {
        current_layer = WIN_BASE;
    }
    if (layer_state_cmp(state, WIN_FN)) {
        current_layer = WIN_FN;
    }
#ifdef CONSOLE_ENABLE
